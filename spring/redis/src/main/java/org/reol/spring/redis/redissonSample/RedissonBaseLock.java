...
public abstract class RedissonBaseLock extends RedissonExpirable implements RLock {
    ...
    private void renewExpiration() {
        ...
        // 超时任务 10秒后执行
        // internalLockLeaseTime / 3  也就是锁的三分之一时间 也就是十s 看门狗的默认释放时间是30s
    ...
    protected void scheduleExpirationRenewal(long threadId) {
        ...
        // 创建一个新的 entry 放到这个 EXPIRATION_RENEWAL_MAP
        // （putIfAbsent方法标识如果有，那就不存，同时返回已经存在的entry，也就是重入的情况）
        ExpirationEntry oldEntry = EXPIRATION_RENEWAL_MAP.putIfAbsent(getEntryName(), entry);
        if (oldEntry != null) {// 如果旧的entry存在 那么说明是重入的情况
            oldEntry.addThreadId(threadId);
        } else {
            // 如果旧的entry等于null 那么就是第一次创建
    ...
    protected void cancelExpirationRenewal(Long threadId) {
        ...
        // 取消定时任务 同时从 EXPIRATION_RENEWAL_MAP 删除任务
